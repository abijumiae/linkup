"use client";

import Link from "next/link";
import { Compass, Play } from "lucide-react";

type WatchEmptyStateProps = {
  onDropMoment?: () => void;
};

export default function WatchEmptyState({ onDropMoment }: WatchEmptyStateProps) {
  return (
    <div className="linkup-panel border-dashed p-10 text-center">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-brand-primary/10 text-brand-primary dark:text-brand-secondary">
        <Play className="h-5 w-5" />
      </div>
      <h3 className="mt-4 text-base font-semibold text-slate-900 dark:text-white">
        No videos to show
      </h3>
      <p className="mx-auto mt-2 max-w-md text-sm text-slate-600 dark:text-slate-400">
        Explore Discover or drop your own Moments.
      </p>
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        <Link
          href="/explore"
          className="linkup-btn-secondary inline-flex min-h-[44px] items-center gap-2"
        >
          <Compass className="h-4 w-4" />
          Explore Discover
        </Link>
        {onDropMoment ? (
          <button type="button" onClick={onDropMoment} className="linkup-btn-primary min-h-[44px]">
            Drop Moment
          </button>
        ) : null}
      </div>
    </div>
  );
}
