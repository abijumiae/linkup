      ...
      <div className="mx-auto max-w-[1760px] px-4 py-6 sm:px-6 lg:px-8">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          <main className="space-y-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-slate-900/80 dark:shadow-slate-950/20">
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-violet-600 dark:text-violet-300/80">
                Today&apos;s Pulse
              </p>
                ...
                {todaysPulseCards.map((card) => {
                  ...
                  return (
                    <Link
                      key={card.label}
                      href={card.href}
                      className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-violet-400/30 hover:bg-violet-500/5 dark:border-white/10 dark:bg-slate-950/80 dark:hover:bg-violet-500/10"
                    >
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-slate-900/80 dark:shadow-slate-950/20">
              ...
              <textarea
                ...
                className="mt-5 min-h-[120px] w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-violet-400/60 dark:border-white/10 dark:bg-slate-950/80 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-violet-400/50"
            ...
            <section
              id="fresh-drops"
              className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-slate-900/80 dark:shadow-slate-950/20"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-violet-600 dark:text-violet-300/80">
                Fresh Drops
              </p>
              ...
              <div className="mt-6 space-y-4">
                {posts.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-white/15 dark:bg-slate-950/60">
                  ...
                  posts.map((post) =>
                    post.apiPost && !post.isStatic ? (
                      ...
                      <article
                        key={post.id}
                        className="rounded-2xl border border-slate-200 bg-white p-5 shadow-lg dark:border-white/10 dark:bg-slate-900/80"
                      >
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                          <div className="flex items-center gap-3">
                            <div className="flex h-12 w-12 items-center justify-center rounded-3xl bg-gradient-to-br from-violet-500 to-sky-500 text-sm font-semibold text-white">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-slate-900/80">
              ...
              <div className="mt-4 space-y-3">
                {homeSuggestions.length === 0 ? (
                  <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600 dark:border-white/15 dark:bg-slate-950/60 dark:text-slate-400">
                  ...
                  homeSuggestions.map((name) => (
                    <div
                      key={name}
                      className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-slate-950/80"
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-sky-500 text-sm font-semibold text-white">
                          {getInitials(name)}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-900 dark:text-white">
                      ...
                      <Link
                        href="/explore"
                        className="rounded-full bg-gradient-to-r from-violet-600 to-sky-600 px-3 py-1.5 text-xs font-semibold text-white shadow-md shadow-violet-600/20 transition hover:from-violet-500 hover:to-sky-500"
                      >
                        Connect
                      </Link>
                    </div>
                ...
                {opportunityCards.map((card) => {
                  ...
                  return (
                    <Link
                      key={card.title}
                      href={card.href}
                      className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-violet-400/30 hover:bg-violet-500/5 dark:border-white/10 dark:bg-slate-950/80 dark:hover:bg-violet-500/10"
                    >
